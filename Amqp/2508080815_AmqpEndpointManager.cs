...
namespace AmqpModbusIntegration
{
	public class AmqpEndpointManager : IDisposable
	{
		...
		//	// StartAmqpEndpoint() 第一次呼叫  endpoint.Open(...)   之前插入
		//	// ── PATCH-A  StartAmqpEndpoint() ─────────────────────────
		//	Amqp.Trace.TraceLevel = Amqp.TraceLevel.Frame;   // 0=Off,1=Info,2=Frame,3=Verbose
		//	Amqp.Trace.TraceListener = (lvl, fmt, arg) =>    // ← 三個參數！
		//	{
		//		var txt = string.Format(fmt, arg);
		...
		//	// ─────── PATCH-B : LinkAttached 之後才開 PublishLoop ───────
		//	endpoint.OnLinkEvent += (linkEvent, link, info) =>
		//	{
		//		if (linkEvent == LinkEvent.LinkAttached)
		//		{
		//			// event / control /（可選）subscribe 三條都 Attach 才算 ready
		//			if (endpoint.Channels.All(ch => ch.IsAttached))
		//			{
		//				_reconnectedAtUtc = DateTime.UtcNow;
		//				_amqpConnected = true;

		//				PublishSafe(new EndpointConnected());
		//				TryStartPublishLoop();   // ← 原本在 HandleConnectionEvent
		//			}
		//		}
		//	};
		//	// ───────────────────────────────────────────────────────
		...
		//	_idleWatchdogCts?.Cancel();         // ⬅︎★ NEW：先關掉舊 watchdog
		//	_idleWatchdogCts?.Dispose();
		//	_idleWatchdogCts = new CancellationTokenSource(); // ⬅︎★ NEW
		//	_ = Task.Run(IdleWatchdogLoop, _idleWatchdogCts.Token);


		//	// ← 加這一行
		//	//DebugObjectDumper.Dump(endpoint, "endpoint");
		//	// 任何你確定 endpoint 已經 Open 之後的地方──例如 StartAmqpEndpoint 尾端
		//	//SocketFinder.DumpSockets(endpoint, "endpoint");
		//	// ── PATCH-3  （只留在測試版，release 註解掉） ──
		...
		public void StartAmqpEndpoint(string endpointName)
		{
			...
			// ------- Open + 加通道 ----------
			endpoint.Open(endpointName, new Uri(endpointUri));
			endpoint.AddPublishChannel(new Uri(publishChannelUri), "eqpevent");

			//這個會導致沒辦法發出去
			//// 2) 取代原本的 "event" 寫法 —— 以 URI 的 path 當作 address
			//// -----------------------------------------------------------
			//var pub = new Uri(publishChannelUri);

			//// 連線只用 scheme/host/port（例如 amqp://10.0.0.1:5672）
			//var pubBroker = new Uri($"{pub.Scheme}://{pub.Host}:{pub.Port}");

			//// 目標地址用 path（去掉開頭的 /），例如 amq.topic、CFX、some.exchange
			//var pubAddress = pub.AbsolutePath.Trim('/');

			//// 沒有 path 的話，依你們環境給預設值（amq.topic 很常見；你們若是 CFX 就填 CFX）
			//if (string.IsNullOrEmpty(pubAddress))
			//	pubAddress = "amq.topic";   // ← 改成你們實際用的預設

			//endpoint.AddPublishChannel(pubBroker, pubAddress);
			//AppLogger.Info($"[AMQP] 發佈通道：Broker={pubBroker}, Address={pubAddress}");
			//// -----------------------------------------------------------

			//// 3)（可選）只有真的有填 SubscribeAddress 才建立訂閱通道
			//if (!string.IsNullOrWhiteSpace(subscribeChannelUri))
			//{
			//	var sub = new Uri(subscribeChannelUri);
			//	var subBroker = new Uri($"{sub.Scheme}://{sub.Host}:{sub.Port}");
			//	var subAddress = sub.AbsolutePath.Trim('/');
			//	if (string.IsNullOrEmpty(subAddress))
			//		subAddress = "subscribe";  // 有固定名字就改這裡

			//	endpoint.AddSubscribeChannel(subBroker, subAddress);
			//	AppLogger.Info($"[AMQP] 訂閱通道：Broker={subBroker}, Address={subAddress}");
			//}
			//else
			//{
			//	AppLogger.Info("[AMQP] 未設定 SubscribeAddress，本程式不建立下行訂閱通道。");
			//}
