...
namespace AmqpModbusIntegration
{
	...
	public class AmqpEndpointManager
	{
		...
		private static readonly Dictionary<int, (int Code, string ErrorDescription)> faultDictionary = new Dictionary<int, (int Code, string ErrorDescription)>
{
	{ 0, (1, "A相過壓") },
	{ 1, (2, "B相過壓") },
	{ 2, (3, "C相過壓") },
	...
	{ 26, (27, "過溫預警") }
		...
		private void PublishStationParametersModifiedMessages(byte stationNumber, AmqpCFXEndpoint endpoint)
		{
			...
			// 提取三相溫度的最大值
			int maxTemperature = 0; // 預設為0
			if (data.ContainsKey("OV_MainTemperatureA") || data.ContainsKey("OV_MainTemperatureB") || data.ContainsKey("OV_MainTemperatureC"))
			{
				maxTemperature = new[]
				{
			data.ContainsKey("OV_MainTemperatureA") ? data["OV_MainTemperatureA"] : 0,
			data.ContainsKey("OV_MainTemperatureB") ? data["OV_MainTemperatureB"] : 0,
			data.ContainsKey("OV_MainTemperatureC") ? data["OV_MainTemperatureC"] : 0
		}.Max();
			}
			...
			var parameters = new List<Parameter>
	{
		...
		{
			Name = "OV_SwitchStatusMain",
			Value = data.ContainsKey("OV_SwitchStatusMain") ? data["OV_SwitchStatusMain"].ToString() : "0"
		},
		// 三相溫度
		new GenericParameter
		{
			Name = "OV_MainTemperatureA",
			Value = data.ContainsKey("OV_MainTemperatureA") ? data["OV_MainTemperatureA"].ToString() : "0"
		},

		new GenericParameter
		{
			Name = "OV_MainTemperatureB",
			Value = data.ContainsKey("OV_MainTemperatureB") ? data["OV_MainTemperatureB"].ToString() : "0"
		},

		new GenericParameter
		{
			Name = "OV_MainTemperatureC",
			Value = data.ContainsKey("OV_MainTemperatureC") ? data["OV_MainTemperatureC"].ToString() : "0"
		}
	};
