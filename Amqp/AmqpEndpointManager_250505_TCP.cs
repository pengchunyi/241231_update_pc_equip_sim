...
namespace AmqpModbusIntegration
{
	public class AmqpEndpointManager
	{
		...
		public void StartAmqpEndpoint(string endpointName)
		{
			...
			// 修正後 (Lambda 明確委託)
			endpoint.OnRequestReceived += (CFXEnvelope req) =>
			{
			...
			Task.Run(async () =>
			{
				...
				//250505新增==========
				TcpClient modbusClient = new TcpClient();  // 新增TCP客戶端實例[3,4](@ref)
				//250505新增==========
						...
						if ((DateTime.Now - lastEnergyPublishTime).TotalSeconds >= 60)
						{
							foreach (var stationNumber in slaveData.Keys)
							{
								try
								{
									...
									{
										await modbusClient.ConnectAsync("192.168.1.200", 502); // 網關默認IP
										await Task.Delay(100); // 等待連接穩定
									}

									// 構造MODBUS TCP請求幀[7](@ref)
									byte[] request = BuildModbusRequest(stationNumber, 0x03, 0x0000, 0x0030);
									NetworkStream stream = modbusClient.GetStream();
									...
									// 解析MODBUS TCP響應（跳過7字節MBAP頭）[7](@ref)
									if (bytesRead > 7 && ValidateResponse(response, bytesRead))
									{
										// 修正後 (補充 modbusViewer 參數)
										ModbusHelper.ParseModbusData(response, 7, stationNumber, modbusViewer);
								...
								{
									Console.WriteLine($"TCP通信異常: {ex.Message}");
									modbusClient.Dispose();
									modbusClient = new TcpClient(); // 重置連接
								}
							}
		...
		// MODBUS TCP請求幀構建（新增方法）
		private byte[] BuildModbusRequest(byte stationId, byte functionCode, ushort startAddress, ushort registerCount)
		{
			...
			request[4] = 0x00; // Length High (剩余字節數)
			request[5] = 0x06; // Length Low (6 bytes after)
							   // PDU部分
			request[6] = stationId;
		...
		private bool ValidateResponse(byte[] response, int length)
		{
			...
			return response.Length >= 9 &&
				   response[7] == 0x03 && // 功能碼03
				   (response[8] & 0x80) == 0; // 無異常標誌
		}
								...
								if (param.Value.ToString() == "1" || param.Value.ToString() == "2")
								{
									...
									foreach (var stn in stationNumbers)
									{
										...
										try
										{
											if (isSwitchOn)
											{
												modbusViewer.ExecuteSwitchCommand((ip, port, station) =>
												{
													ModbusHelper.SwitchON(modbusViewer.ip, modbusViewer.port, station);
												}, new List<byte> { stn });
											...
											else
											{
												modbusViewer.ExecuteSwitchCommand((ip, port, station) =>
												{
													ModbusHelper.SwitchOFF(modbusViewer.ip, modbusViewer.port, station);
												}, new List<byte> { stn });
											}
											hasProcessed = true;
										}
