...
namespace AmqpModbusIntegration.Services
{
	...
	public class ConnectionManager
	{
		...
		//		try
		//		{
		//			await AutoReconnectLoopAsync(ct); // 確保 COM 已開啟且可讀（若未配置站號，開啟即視為就緒）
		...
		//				if (!ok) throw new Exception("首次讀取失敗");
		//			}
		//			else
		//			{
		//				AppLogger.Info("[MODBUS] 尚未設定站號，已開啟串口，等待後續資料/設定…");
		//				ok = true;
		//			}
		...
		//		catch (Exception ex) when (!ct.IsCancellationRequested)
		//		{
		//			AppLogger.Warn($"[MODBUS] 首次連線重試：{ex.Message}");
		//		}
		...
		//250730_備分
		///// <summary>
		///// 只嘗試設定檔指定的 COM；每 2 秒重試一次，直到成功或取消。
		///// - 若同一埠且已開啟 → 不重建 SerialPort，直接驗證讀取（避免重覆「已成功連接」log）
		///// - 若埠不存在 → 等待裝置回來
		///// - 有設定站號：成功讀取一次即視為就緒；無站號：只要串口開啟即視為就緒
		///// </summary>
		...
		//				if (ok) return; // 成功 → 跳出重連 loop
		//				else AppLogger.Warn($"[SERIAL] {configured} 已開啟，但讀取失敗，續試中…");
		//			}
		//		}
		//		catch (Exception ex)
		//		{
		...
		///// <summary>
		///// 自動重連：
		///// - ManualComOverride=true：只看「目前已開啟的埠」或「UI 選到的埠」，完全忽略 ini 的 COM
		///// - ManualComOverride=false：遵循 ini 的 COM
		///// </summary>
		...
		//					bool okNow = hasStationsNow ? 
		//						await ModbusHelper.ReadAllParametersAsync(current, _viewer) : true;
		//					if (okNow) return 
		//							true;
		...
		//					{
		//						// 手動模式下，避免刷不存在的 ini COM 訊息；這裡也不刷 selected 不存在的訊息
		//						await Task.Delay(1000, ct);
		//						continue;
		//					}
