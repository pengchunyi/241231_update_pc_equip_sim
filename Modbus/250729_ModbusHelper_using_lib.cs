...
////				}
////				viewer.AppendLog($"[MODBUS] 站號 {station} 回應 header 不足：{available}/3 bytes（逾時 {readTimeout}ms）");
////				return false;
////			}
...
////			if (gotHeader != 3)
////			{
////				viewer.AppendLog($"[MODBUS] 站號 {station} Header 讀取長度不符：{gotHeader}/3");
////				return false;
////			}
...
////			{
////				viewer.AppendLog($"[MODBUS] 站號 {station} 回應頭不符：addr={addr}, func={func}, bc={bc}");
////				// 讀掉剩餘資料以免阻塞
...
////				}
////				viewer.AppendLog($"[MODBUS] 站號 {station} 回應長度不足：{availableRemain}/{remain} bytes（逾時 {readTimeout}ms）");
////				return false;
////			}
...
////			if (gotTail != remain)
////			{
////				viewer.AppendLog($"[MODBUS] 站號 {station} 回應尾段長度不符：{gotTail}/{remain}");
////				return false;
////			}
...
////				viewer.AppendLog($"[MODBUS] 站號 {station} CRC 校驗失敗（len={buffer.Length}）head={head}");
////				return false;
////			}
...
////			// 這裡依舊假設連續 48 regs（bc=96），故以下索引與你原本固定長度一致
////			if (bc < 96) return; // 防守式
