...
namespace AmqpModbusIntegration
{
    public static class ModbusHelper
    {
		...
		public static async Task SwitchONAsync(IModbusMaster master, byte stationNumber)
		{
			try
			{
				ushort registerAddress = 49;
				ushort value = 21862;
				await master.WriteSingleRegisterAsync(stationNumber, registerAddress, value);
				Console.WriteLine($"Station {stationNumber}: Switch opened.");
			}
			catch (Exception ex)
			{
				Console.WriteLine($"Error switching ON station {stationNumber}: {ex.Message}");
			}
		}
		...
		public static async Task SwitchOFFAsync(IModbusMaster master, byte stationNumber)
		{
			try
			{
				ushort registerAddress = 49;
				ushort value = 21896;
				await master.WriteSingleRegisterAsync(stationNumber, registerAddress, value);
				Console.WriteLine($"Station {stationNumber}: Switch closed.");
			}
			catch (Exception ex)
			{
				Console.WriteLine($"Error switching OFF station {stationNumber}: {ex.Message}");
			}
		}
  ...
  //      }
		...
		public static async Task ReadAllParametersAsync(IModbusMaster master, ModbusViewer modbusViewer)
		{
			try
			{
				foreach (var station in modbusViewer.GetSlaveData().Keys.ToList())
				{
					...
					ushort numRegisters = 25;
					ushort unitId = station; // 假設站號即為單元ID

					ushort[] registers = await master.ReadHoldingRegistersAsync(unitId, startAddress, numRegisters);
					Console.WriteLine($"Station {station} Registers: {string.Join(", ", registers)}");

					// 更新 slaveData
					var data = new Dictionary<string, int>
			{
				{ "a_temperature", registers[0] },
				{ "b_temperature", registers[1] },
				{ "c_temperature", registers[2] },
				{ "a_current", registers[4] / 10 },
				{ "b_current", registers[5] / 10 },
				{ "c_current", registers[6] / 10 },
				{ "a_power", registers[13] },
				{ "b_power", registers[14] },
				{ "c_power", registers[15] },
				{ "count_switch", registers[19] },
				{ "total_power", registers[20] }
			};

					modbusViewer.GetSlaveData()[station] = data;
				}
			}
			catch (Exception ex)
			{
