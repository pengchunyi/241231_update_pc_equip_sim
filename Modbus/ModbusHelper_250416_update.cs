...
namespace AmqpModbusIntegration
{
	public static class ModbusHelper
	{
		...
		//鎖頭，必免陷入爭搶資源
		//a.統一鎖物件（全域共享）
		//原本此檔案中使用私有的 SemaphoreSlim 來鎖定，但其他檔案則使用不同的鎖。
		//建議將鎖公開，讓所有操作均使用此同一個鎖。
		public static readonly SemaphoreSlim serialPortLock = new SemaphoreSlim(1, 1);

		//b.更新讀寫操作以使用 SemaphoreSlim
		//對於同步方法（如 SwitchON、SwitchOFF、SetTemperature）
		//採用同步的 Wait()/Release()；而對於非同步方法（如 ReadAllParametersAsync）
		//則採用 await SerialPortLock.WaitAsync()／Release()。
