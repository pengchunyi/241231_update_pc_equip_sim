...
//	{
//		// 與原始碼保持一致
//		private static readonly SemaphoreSlim serialPortLock = new(1, 1);

//		#region Switch ON / OFF
//		public static void SwitchON(SerialPort sp, byte stn)
//		{
//			lock (serialPortLock)
//			{
//				if (sp == null || !sp.IsOpen) return;
//				sp.RtsEnable = true;
//				byte[] cmd = { stn, 0x06, 0x00, 0x31, 0x55, 0x88 };
//				Send(sp, cmd);
//				Console.WriteLine($"站號 {stn} 合閘");
//			}
//		}
//		public static void SwitchOFF(SerialPort sp, byte stn)
//		{
//			lock (serialPortLock)
//			{
//				if (sp == null || !sp.IsOpen) return;
//				sp.RtsEnable = true;
//				byte[] cmd = { stn, 0x06, 0x00, 0x31, 0x55, 0x66 };
//				Send(sp, cmd);
//				Console.WriteLine($"站號 {stn} 分閘");
//			}
//		}
//		#endregion

//		#region Temperature & Fault test (保持您完整邏輯)
//		public static void SetTemperature(SerialPort sp, byte stn, ushort temp,
//			Dictionary<byte, Dictionary<string, object>> slaveData)
//		{
//			// 與您原始碼完全一致 – 省略註解
//			if (sp == null || !sp.IsOpen) { Console.WriteLine("串口未開啟"); return; }
//			byte[] cmd = { stn, 0x06, 0x00, 0x2B, (byte)(temp >> 8), (byte)(temp & 0xFF) };
//			Send(sp, cmd);
//			if (slaveData.ContainsKey(stn)) slaveData[stn]["TemperatureSV"] = temp;
//		}

//		public static void SimulateFaultTest(ModbusViewer viewer,
//			Dictionary<byte, Dictionary<string, object>> slaveData)
//		{
//			int faultCode = 0xA87;
//			viewer.currentStatus = faultCode;
//			viewer.currentStatus1 = (faultCode >> 16) & 0xFFFF;
//			viewer.currentStatus2 = faultCode & 0xFFFF;
//			foreach (var st in slaveData.Keys) slaveData[st]["Fault_WarningCode"] = faultCode;
//			viewer.UpdateDataGridView();
//		}
//		#endregion

//		#region ReadAllParametersAsync (取樣保持完整)
//		public static async Task ReadAllParametersAsync(SerialPort sp, ModbusViewer viewer)
//		{
//			if (sp == null || !sp.IsOpen) return;
//			foreach (byte stn in viewer.SlaveData.Keys.ToList())
//			{
//				try
//				{
//					byte[] cmd = { stn, 0x03, 0x00, 0x00, 0x00, 0x30 };
//					Send(sp, cmd, true/*noCRC*/);
//					await Task.Delay(300);
//					byte[] buf = new byte[256]; int len = sp.Read(buf, 0, buf.Length);
//					if (len > 5 && ValidateCRC(buf, len))
//						ParseResponse(buf, stn, viewer);
...
//		#region 低階工具
//		private static void Send(SerialPort sp, byte[] cmd, bool appendCRC = false)
//		{
//			byte[] full = appendCRC ? AppendCRC(cmd) : cmd;
//			sp.DiscardInBuffer(); sp.DiscardOutBuffer();
//			sp.Write(full, 0, full.Length);
//			Thread.Sleep(10); sp.RtsEnable = false;
//		}

//		public static byte[] AppendCRC(byte[] cmd)
//		{
//			ushort crc = CalculateCRC(cmd);
//			byte[] full = new byte[cmd.Length + 2];
//			Array.Copy(cmd, full, cmd.Length);
...
//			return crc;
//		}
//		public static bool ValidateCRC(byte[] data, int len)
//		{
//			if (len < 2) return false;
//			ushort recv = (ushort)(data[len - 2] | (data[len - 1] << 8));
//			ushort calc = CalculateCRC(data.Take(len - 2).ToArray());
//			return recv == calc;
//		}
//		#endregion

//		#region ParseResponse (保持原算法 – 省略, 請貼原完整函數)
//		//private static void ParseResponse(byte[] buf, byte stn, ModbusViewer viewer)
//		//{
//		//	// ---- 直接複製您 250508_configVer_update.cs 中 ParseResponse ----
//		//}
