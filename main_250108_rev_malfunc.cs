...
namespace AmqpModbusIntegration  // 命名空間，用於AMQP（高級消息隊列協議）和Modbus整合的程序
{
	...
	public class ModbusViewer : Form
	{
		...
		private void InitializeTimer()
		{
			...
			// 绑定事件
			updateTimer.Tick += OnTimerElapsed;
		...
		private async void UpdateValues()
		{
			...
			{
				Console.WriteLine($"更新值时发生错误: {ex.Message}");
			}
			finally
			{
	...
	internal class Program
	{
		...
		{
			// 初始化 ModbusViewer
			var modbusViewer = new ModbusViewer();

			// 在主執行緒中啟動視窗
			var uiTask = Task.Run(() => Application.Run(modbusViewer));

			// 啟動 AMQP 和 Kafka 的任務
			await InitializeBackgroundTasks(modbusViewer);
		...
		static async Task InitializeBackgroundTasks(ModbusViewer modbusViewer)
		{
			// 啟動 AMQP 模塊
			...
			//var amqpTask = Task.Run(async () => await amqpManager.StartAmqpEndpoint("CHUNYI_PC")).ConfigureAwait(false);
			var amqpTask = Task.Run(() => amqpManager.StartAmqpEndpointAsync("CHUNYI_PC"));
			...
			{
				BootstrapServers = "10.146.206.41:9092",
			...
			// 啟動 Kafka 模塊
			var kafkaTask = Task.Run(async () =>
			{
				using (var kafkaProducer = new ProducerBuilder<string, string>(producerConfig).Build())
				{
					var kafkaManager = new KafkaManager(kafkaProducer, "KafkaTopic", modbusViewer);
					await kafkaManager.StartPublishingAsync().ConfigureAwait(false);
				}
			});

			// 等待所有後台任務完成
			await Task.WhenAll(amqpTask, kafkaTask);
		}
