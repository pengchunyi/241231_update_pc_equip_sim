    ...
    public class ModbusViewer : Form
    {
        ...
        public ModbusViewer()
        {
			...
			connectButton.Click += (s, e) =>
			{
				...
				InitializeTcpClient(ipAddress, port);
			};
		...
		private void InitializeTcpClient(string ipAddress, int port)
		{
			try
			{
				tcpClient = new TcpClient(); connectButton.Click
				tcpClient.Connect(ipAddress, port);
				var factory = new ModbusFactory();
				master = factory.CreateMaster(tcpClient);
				Console.WriteLine($"Connected to Modbus TCP server at {ipAddress}:{port}");
			}
			catch (Exception ex)
			{
				Console.WriteLine($"Failed to connect to Modbus TCP server: {ex.Message}");
			}
		}
		...
		static void Main()
		{
			...
			modbusViewer.Connected += () =>
			{
				var amqpManager = new AmqpEndpointManager(
					...
					"amqp://127.0.0.1:6666",
					modbusViewer,
					modbusViewer.IpAddress, // 假設有此屬性
					modbusViewer.Port,      // 假設有此屬性
