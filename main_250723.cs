...
namespace AmqpModbusIntegration
{
	...
	internal class Program
	{
		...
		//		/* ★ 新增：先初始化 SerialPort，把 Port 打開 -------------------- */
		//		modbusViewer.InitializeSerialPort(SwitchDeviceConfig.ComPort);
		//		/* -------------------------------------------------------------- */


		//		// 取得 modbusViewer 中的 slaveData
		//		var serialPort = modbusViewer.GetSerialPort(); // ← 現在一定不是 null
		...
		//		amqpManager.StartAmqpEndpoint(SystemConfig.MachineSN ?? "failed to load CFX.ini MachineSN");


		//		 // ★ 建立並啟動連線 / 重試 / 上報 管理器
		//		_connMgr = new ConnectionManager(serialPort, slaveData, amqpManager);
		//		_connMgr.Start();
		...
		static void Main()
		{
			...
			using (var mutex = new Mutex(true, @"Global\CFX_SmartBreaker_Upload_Mutex", out newInstance))
			{
				...
				try
				{
					...
					amqpManager.StartAmqpEndpoint(
						SystemConfig.MachineSN ?? "failed to load CFX.ini MachineSN");
