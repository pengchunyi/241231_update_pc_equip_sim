...
//            // 開關控制按鈕
//            switchButton = new Button { Text = "開啟開關", Location = new System.Drawing.Point(300, 50), Width = 80 };
//            switchButton.Click += (s, e) => SwitchON(stationNumber);
//            Controls.Add(switchButton);

//            // 開關控制按鈕
//            switchButton = new Button { Text = "關閉開關", Location = new System.Drawing.Point(400, 50), Width = 80 };
//            switchButton.Click += (s, e) => SwitchOFF(stationNumber);
//            Controls.Add(switchButton);
...
//        private void SwitchON(byte station)
//        {
//            if (serialPort != null && serialPort.IsOpen)
//            {
//                byte[] openCommand = { station, 0x06, 0x00, 0x31, 0x55, 0x88 }; // 站號替換為參數
//                ushort crc = CalculateCRC(openCommand);
//                byte[] crcBytes = BitConverter.GetBytes(crc);
//                byte[] fullCommand = new byte[openCommand.Length + 2];
//                Array.Copy(openCommand, fullCommand, openCommand.Length);
...
//        private void SwitchOFF(byte station)
//        {
//            if (serialPort != null && serialPort.IsOpen)
//            {
//                byte[] closeCommand = { station, 0x06, 0x00, 0x31, 0x55, 0x66 }; // 站號替換為參數
//                ushort crc = CalculateCRC(closeCommand);
//                byte[] crcBytes = BitConverter.GetBytes(crc);
//                byte[] fullCommand = new byte[closeCommand.Length + 2];
//                Array.Copy(closeCommand, fullCommand, closeCommand.Length);
...
//                                data["B相溫度"] = (buffer[11] << 8) | buffer[12];                 // 當前B相溫度
//                                data["C相溫度"] = (buffer[13] << 8) | buffer[14];                 // 當前C相溫度
...
//                        catch (Exception ex)
//                        {
//                            Console.WriteLine($"讀取站號 {station} 數據時發生錯誤: {ex.Message}");
//                        }
//                    });
//                }
//            }
...
//                //Uri = new Uri("amqp://10.181.56.175:30031")//這是SIE的IP
//            });
